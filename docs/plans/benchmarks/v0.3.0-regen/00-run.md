# Kuberina benchmark run — proof-of-work for PAPER.md §6–§7

Recorded after the IR schema fix: the generator was emitting the I/O dimensions
flat (`disk_read`, `net_in`), which the solver silently dropped. Every run before
this one planned in four dimensions. See the commit for details.

- Commit: b52fcc3 (working tree, IR fix applied)
- Date:   2026-09-06T15:50:43+07:00
- Host:   Linux 7.0.0-31-generic x86_64

## Step 1 — regenerate testdata
...
## Step 2 — solver, full packing (no headroom)

```
cd solver && cargo run --release -- plan \
--infra testdata/irina_infra.yaml \
--workloads testdata/irina_workloads.yaml
   Compiling kuberina-solver v0.2.0 (/home/stella/workspace/kuberina/solver)
    Finished `release` profile [optimized] target(s) in 4.09s
     Running `target/release/kuberina-solver plan --infra testdata/irina_infra.yaml --workloads testdata/irina_workloads.yaml`
...
Phase 1 (FFD): seed fitness = 1582583.1142
...
═══ Final Blueprint (Stowage Plan) ═══
  ...
  Scorecard:
    ...
    Gang Penalty: 0
    Active Nodes: 540
    Fragmentation: 1577120.00
    Affinity Violations: 0
    Utilization Variance: 0.0571
    Topology Spread Penalty: 21.00

  ─── Cluster Summary (620 nodes) ───
  Active nodes: 540 / 620
  Empty nodes:  80
  Pods placed:  2714
  Avg CPU util: 29.1%

  ─── Top 5 Busiest Nodes ───
  mem-029: 29.0/29.8 CPU (97%), 10 pods
  mem-030: 29.0/29.8 CPU (97%), 13 pods
  mem-031: 29.0/29.8 CPU (97%), 13 pods
  mem-032: 29.0/29.8 CPU (97%), 13 pods
  mem-033: 29.0/29.8 CPU (97%), 13 pods

  ─── Top 5 Lightest Active Nodes ───
  std-310: 6.0/61.8 CPU (10%), 3 pods
  std-311: 6.0/61.8 CPU (10%), 3 pods
  std-312: 6.0/61.8 CPU (10%), 3 pods
  std-313: 6.0/61.8 CPU (10%), 3 pods
  std-314: 6.0/61.8 CPU (10%), 3 pods

  ─── 80 nodes empty (available for shutdown) ───

  (Full stowage plan exported to kuberina_solution.yaml)
```

## Step 3 — inspector, full packing
...
## Step 4 — mathematical proof, full packing
...
──────────────────────────────────────────────────────────────────────
  ...
  Objective — Topology Spread:
    Soft Penalty (Total Skew): 2100.00
...
──────────────────────────────────────────────────────────────────────
  ...
  Kuberina used: 540 active nodes
    α vs real-capacity bound     = 540/353 = 1.5297

  ══ α = 1.5297 — above FFD 1D guarantee, expected for multi-D ══
...
======================================================================
  ...
  2. Approx ratio:   α = 1.5297 (LB = 353 nodes, against real capacity)
  3. Significance:   p < 1.0e-04
======================================================================
```

## Step 5 — solver, 20% headroom reserved
...
--headroom 20
    Finished `release` profile [optimized] target(s) in 0.10s
     Running `target/release/kuberina-solver plan --infra testdata/irina_infra.yaml --workloads testdata/irina_workloads.yaml --headroom 20`
Loaded 620 nodes, 4 daemonsets, 2714 pods, 0 groups
...
Phase 1 (FFD): seed fitness = 414402512.4033
Datacenter-scale detected (2714 pods) — cranking GA to maximum
Early stop at generation 199 (no improvement for 200 gens)

═══ Final Blueprint (Stowage Plan) ═══
  Fitness: 414402512.4033
  Time: 221.99s
  Scorecard:
    Capacity Penalty: 412668000
    Selector Penalty: 0
    Gang Penalty: 0
    Active Nodes: 581
    Fragmentation: 1728633.20
    Affinity Violations: 0
    Utilization Variance: 0.1016
    Topology Spread Penalty: 23.00

  ─── Cluster Summary (620 nodes) ───
  Active nodes: 581 / 620
  Empty nodes:  39
  Pods placed:  2714
  Avg CPU util: 27.8%

  ─── Top 5 Busiest Nodes ───
  mem-000: 96.0/29.8 CPU (323%), 16 pods
  gpu-000: 36.0/45.8 CPU (79%), 9 pods
  gpu-001: 36.0/45.8 CPU (79%), 9 pods
  gpu-002: 36.0/45.8 CPU (79%), 9 pods
  gpu-003: 36.0/45.8 CPU (79%), 9 pods

  ─── Top 5 Lightest Active Nodes ───
  gpu-030: 3.0/45.8 CPU (7%), 1 pods
  gpu-031: 3.0/45.8 CPU (7%), 1 pods
  gpu-032: 3.0/45.8 CPU (7%), 1 pods
  gpu-033: 3.0/45.8 CPU (7%), 1 pods
  gpu-034: 3.0/45.8 CPU (7%), 1 pods

  ─── 39 nodes empty (available for shutdown) ───

  (Full stowage plan exported to kuberina_solution.yaml)
```

## Step 6 — inspector, 20% headroom
...
--- Validation Results ---
Selector Violations: 0
Capacity Overflow (CPU): 66.25
Capacity Overflow (RAM): 306.75
Capacity Overflow (GPU): 0.00
Capacity Overflow (Storage): 1508.50
Capacity Overflow (Disk Read): 4821.00
Capacity Overflow (Disk Write): 11328.00
Capacity Overflow (Net In): 10570.00
Capacity Overflow (Net Out): 10605.00
Unassigned Pods: 0
--------------------------
...
## Step 7 — mathematical proof, 20% headroom
...
──────────────────────────────────────────────────────────────────────
  ...
  Predicate 1 — Capacity:
    ∀j ∈ N: Σᵢ xᵢⱼ · reqᵢʳ ≤ Cⱼʳ
    CPU overflow: 66.250000
    RAM overflow: 306.750000
    GPU overflow: 0.000000
    STORAGE overflow: 1508.500000
    DISK_READ overflow: 4821.000000
    DISK_WRITE overflow: 11328.000000
    NET_IN overflow: 10570.000000
    NET_OUT overflow: 10605.000000
    Verdict: ❌ VIOLATED
  ...
  Objective — Topology Spread:
    Soft Penalty (Total Skew): 2300.00
    Verdict: ⚠️ SOFT PENALTY APPLIED
...
──────────────────────────────────────────────────────────────────────
  ...
  Kuberina used: 581 active nodes
    α vs real-capacity bound     = 581/353 = 1.6459
    α vs reserved-capacity bound = 581/441 = 1.3175
  ...
  ══ α = 1.3175 — above FFD 1D guarantee, expected for multi-D ══
...
======================================================================
  SUMMARY
======================================================================
  1. Feasibility:    FAILED ❌
  2. Approx ratio:   α = 1.3175 (LB = 441 nodes, against reserved capacity)
  3. Significance:   p < 1.0e-04
======================================================================
```
